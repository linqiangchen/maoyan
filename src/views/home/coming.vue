            ...
            <div>
              <ul>
                <li v-for="item in topRated" :key="item.id" @click="toMovie(item.id)">
                  ...
                  <p class="one-line time">{{item.comingTitle}}</p>
                  <div class="mask">
                    <p>{{item.wish}}</p>
                  </div>
                </li>
          ...
          <li class v-for="(item ,index) in showList" :key="index">
            ...
            <div class="show-wrap j-s-a one-line">
              <div class="info one-line">
                ...
                <p class="one-line wish"><i>{{item.wish}}</i> 人想看</p>
                <p class="one-line" v-if="item.star">主演:{{item.star}}</p>
                <p class="one-line">{{item.rt}} 上映</p>
              </div>
              <div class="buy" :class="{want:toggleTime(item.showInfo)}">{{showBtn(item.showInfo)}}</div>
            </div>
          </li>
        </ul>
      </div>
...
export default {
  ...
  computed: {
    ...mapState({
      topRated: (state) => state.home.Expected,
  ...
  created() {
    ...
    this.$store.dispatch("home/loadExpected");
    // loadExpected
    // loadComingMovie
  },
  mounted() {
  ...
  methods: {
    ...
    reflsh() {
      ...
      this.$store.dispatch("home/loadMoreComming", this.offset++);
    },
    log(e) {
      // 
      e.refresh();
